...
func TestGenerateKyberKeyPair(t *testing.T) {
	...
	if len(privateKey) != kyberk2so.Kyber1024SKBytes {
		t.Errorf(
			"Private key has incorrect length: got %d, want %d",
			len(privateKey),
			kyberk2so.Kyber1024SKBytes,
		)
	}
	if len(publicKey) != kyberk2so.Kyber1024PKBytes {
		t.Errorf(
			"Public key has incorrect length: got %d, want %d",
			len(publicKey),
			kyberk2so.Kyber1024PKBytes,
		)
	}
}
...
func TestEncryptKyber(t *testing.T) {
	...
	if len(ciphertext) != kyberk2so.Kyber1024CTBytes {
		t.Errorf(
			"Ciphertext has incorrect length: got %d, want %d",
			len(ciphertext),
			kyberk2so.Kyber1024CTBytes,
		)
	}
	if len(sharedSecret) != kyberk2so.KyberSSBytes {
		t.Errorf(
			"Shared secret has incorrect length: got %d, want %d",
			len(sharedSecret),
			kyberk2so.KyberSSBytes,
		)
	}
}
